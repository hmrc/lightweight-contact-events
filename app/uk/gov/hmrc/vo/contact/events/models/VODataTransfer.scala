...
import ConfirmedContactDetails.toLegacyContact
import uk.gov.hmrc.vo.contact.events.utils.Initialize

case class VODataTransfer(
...
object VODataTransfer:

  implicit val format: OFormat[VODataTransfer] = Json.format[VODataTransfer]

  def apply(ctc: Contact, init: Initialize): VODataTransfer =
    VODataTransfer(
  ...
  private def getEmailAddress(enquiryCategoryMsg: String, init: Initialize): String =
    enquiryCategoryMsg match
      ...
      case _                                              =>
        ...
        throw RuntimeException(s"Email address not found for enquiryCategory : $enquiryCategoryMsg")
