...
class VODataTransferConnector @Inject() (
  ...
  def transfer(dataTransfer: VODataTransfer)(using hc: HeaderCarrier): Future[Try[Int]] = sendJson(Json.toJson(dataTransfer))

  private[connectors] def sendJson(json: JsValue)(using hc: HeaderCarrier): Future[Try[Int]] =
      ...
      .map { r =>
        ...
        r.status match {
          ...
          case status                  =>
            ...
            Failure(RuntimeException("Data transfer service fails with status " + status))
        }
      }
      .recover {
        case ex =>
          logger.warn("Data transfer service fails with exception " + ex.getMessage)
          Failure(RuntimeException("Data transfer service fails with exception " + ex.getMessage))
      }
