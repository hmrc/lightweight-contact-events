...
import uk.gov.hmrc.vo.contact.events.{RequestBuilderStub, SpecBase}
...
class VODataTransferConnectorSpec extends SpecBase:
  ...
  def connector(httpClientV2Mock: HttpClientV2) =
    VODataTransferConnector(httpClientV2Mock, auditService, servicesConfig)

  "VO Data Transfer Connector" when {

    "provided with VO Data Transfer Model" must {
    ...
    "provided with JSON directly" must {
      ...
      "return a failure if the data transfer call throws an exception" in {
        ...
        ).thenReturn(RequestBuilderStub(Left(RuntimeException("failure")), "{}"))
